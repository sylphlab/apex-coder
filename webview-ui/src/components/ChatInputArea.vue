          ...
          new Promise((resolve, reject) => {
            ...
            reader.onload = (e) => {
              ...
              if (result && !props.pendingImageBase64.includes(result)) {
                // Avoid duplicates from parent state
                resolve(result);
              } else {
    ...
    Promise.all(readers)
      .then((results) => {
        ...
        if (validResults.length > 0) {
          ...
          emit("update:pendingImageBase64", [
            ...props.pendingImageBase64,
            ...validResults,
          ]);
        }
...
<template>
  <div class="input-section-container px-3 pt-2 pb-1 bg-nordic-bg">
    ...
    <div
      v-if="pendingImageBase64.length > 0"
      class="pending-images-container mb-2 overflow-x-auto custom-scrollbar pb-2"
    >
      <div class="flex space-x-2">
        <div
          v-for="(imgSrc, index) in pendingImageBase64"
          :key="index"
          class="pending-image-preview relative flex-shrink-0"
        >
          <img
            :src="imgSrc"
            alt="Pending upload"
            class="h-16 w-auto rounded border border-nordic-border shadow-sm object-contain bg-nordic-bg-darker"
          />
          <button
            @click="removePendingImage(index)"
            class="absolute top-0 right-0 -mt-1 -mr-1 p-0.5 bg-nordic-error text-white rounded-full text-xs leading-none z-10"
            title="Remove image"
          >
    ...
    >
      <div class="flex items-end space-x-2 w-full">
        ...
        <textarea
          ...
          :class="{
            'opacity-60 cursor-not-allowed': isLoading || !isModelInitialized,
          }"
        ></textarea>
        ...
        <button
          @click="handleSend"
          :disabled="
            (props.currentInput.trim().length === 0 &&
              pendingImageBase64.length === 0) ||
            isLoading ||
            !isModelInitialized
          "
          ...
          :class="{
            ...
            'bg-nordic-primary':
              ...
              !isLoading &&
              isModelInitialized,
            'bg-nordic-bg-muted text-nordic-text-muted':
              !(
                props.currentInput.trim().length > 0 ||
                pendingImageBase64.length > 0
              ) ||
      ...
      <!-- Model Selector Row -->
      <div
        class="flex justify-start w-full pt-1 mt-1 border-t border-nordic-border/10"
      >
        <button
