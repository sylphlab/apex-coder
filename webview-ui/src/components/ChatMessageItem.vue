...
<template>
  <div class="chat-message">
    ...
    <div v-if="message.role === 'user'" class="flex justify-end">
      <div
        class="max-w-[85%] px-3 py-2.5 rounded-xl shadow-nordic-xs bg-nordic-primary/90 text-white"
      >
        <!-- User Images -->
        <div
          v-if="message.imagesBase64 && message.imagesBase64.length > 0"
        ...
        >
          ...
          >
            <img
              :src="imgSrc"
              alt="User image"
              class="block max-w-[100px] max-h-24 rounded-md object-contain bg-white/10 hover:opacity-80 transition-opacity"
            />
          </button>
        </div>
        <!-- User Text -->
    ...
    <div v-else-if="message.role === 'assistant'" class="flex justify-start">
      <div
        ...
        :class="
          message.error
            ? 'bg-nordic-error/10 text-nordic-error'
            : 'bg-nordic-bg'
        "
      >
        <!-- Model ID -->
        <div class="text-xs text-nordic-text-muted mb-1 flex items-center">
          <span
            v-if="configuredModelId && !message.error"
            class="font-semibold text-nordic-text-secondary"
          >
            {{ configuredModelId }}
          </span>
          <span
            v-else-if="message.error"
            class="font-semibold text-nordic-error"
          >
            Error
          </span>
        </div>
