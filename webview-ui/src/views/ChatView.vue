...
const handleSendFromInput = () => {
  ...
  if ((text || images.length > 0) && isModelInitialized.value && !props.isLoading) {
...
const scrollToBottom = (smooth = false) => {
  nextTick(() => {
    if (chatContainer.value) {
      ...
      const isNearBottom = scrollTarget.scrollHeight - scrollTarget.clientHeight <= scrollTarget.scrollTop + 50; // Adjust threshold as needed
...
watch(() => props.chatMessages, (newVal, oldVal) => {
...
<template>
  <div class="flex flex-col flex-grow overflow-hidden bg-nordic-bg-contrast relative">
    ...
    >
      ...
      <ChatEmptyState
        v-if="props.chatMessages.length === 0 && pendingImageBase64.length === 0"
      />
      <ChatMessageList
        v-else
    ...
    <transition name="modal-fade">
      <div v-if="isImageModalVisible" class="image-modal-overlay fixed inset-0 z-50 flex items-center justify-center bg-black/75" @click.self="closeImageModal">
        <div class="relative max-w-[90vw] max-h-[90vh]">
          <img :src="modalImageSrc" alt="Enlarged image" class="block w-auto h-auto max-w-full max-h-full object-contain rounded shadow-lg">
          <button @click="closeImageModal" class="absolute top-2 right-2 p-1 bg-nordic-bg/50 hover:bg-nordic-bg/80 text-white rounded-full text-xs leading-none" title="Close image">
...
.custom-scrollbar::-webkit-scrollbar-thumb { background: var(--nordic-border, #4c566a); border-radius: 3px; }
.custom-scrollbar::-webkit-scrollbar-thumb:hover { background: var(--nordic-text-muted, #6a7388); }

.image-modal-overlay { backdrop-filter: blur(3px); }
