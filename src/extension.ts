...
function getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
	console.log('[Apex Coder] Generating webview content...');

	// Path to the build output directory of the Vue app
	...
	// Replace asset paths (CSS, JS) with webview URIs
	// This regex approach is more robust to handle hashed filenames generated by Vite/Vue build
	// Corrected regex to match paths starting with /assets/
	html = html.replace(/(href|src)="(\/assets\/[^"]+)"/g, (match, p1Attribute, p2Path) => {
		// p2Path will be like "/assets/index.js", remove leading slash for joinPath
	...
	// Add CSP meta tag
    const nonce = getNonce(); // Function to generate nonce (see below)
    html = html.replace(
        '<head>',
        // Adjusted CSP: Keep nonce, explicitly add webview.cspSource to script/style, add connect/font
        `<head>
        <meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}' ${webview.cspSource}; img-src ${webview.cspSource} data:; connect-src ${webview.cspSource}; font-src ${webview.cspSource};">`
    );
	...
	console.log('[Apex Coder] Webview content generated successfully.');
	return html;
}
