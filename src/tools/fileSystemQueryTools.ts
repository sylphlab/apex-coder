...
export const fileStatsToolDefinition = {
  ...
  parameters: z.object({
    path: z
      .string()
      .describe(
        'Path relative to workspace root (e.g., "package.json" or "src/utils")',
      ),
  }),
  /**
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          throw new Error(
            `File or directory not found at path: ${relativePath}`,
          );
        }
      ...
      // Use unknown
      const { returnPayload } = handleToolError(
        error,
        "fileStats",
        currentPanel,
        args,
...
export const searchFilesToolDefinition = {
  ...
  parameters: z.object({
    directoryPath: z
      .string()
      .describe(
        'Directory path relative to workspace root to search within (e.g., "src").',
      ),
    pattern: z
      .string()
      .describe(
        'JavaScript-compatible Regex pattern string to search for (e.g., "const\\s+\\w+\\s*=").',
      ),
    fileGlobPattern: z
      .string()
      .optional()
   ...
   */
  async execute(
    args: { directoryPath: string; pattern: string; fileGlobPattern?: string },
    currentPanel?: vscode.WebviewPanel,
    ...
    try {
      ...
      try {
        ...
        if (stat.type !== vscode.FileType.Directory) {
          throw new Error(
            `Search path "${relativeDirPath}" is not a directory.`,
          );
        }
      } catch (statError: unknown) {
        // Use unknown
        if (
          ...
          throw new Error(
            `Search directory not found at path: ${relativeDirPath}`,
          );
        }
        throw statError; // Re-throw other stat errors
      }

      const relativeGlobPattern = new vscode.RelativePattern(
        absoluteDirPathUri,
        fileGlobPattern,
      );
      ...
      const successMsg = `Search complete. Found ${results.length} matches in ${files.length} files scanned.`; // Ensure numbers are used correctly
      logger.info(`[Tool] ${successMsg}`);
      const resultPayload: FilesystemResultPayload = {
